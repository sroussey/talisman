...
import {lig2, lig3} from '../../src/metrics/lig.js';

describe('lig', function() {

  it('should correctly compute the LIG1, LIG2 & LIG3 distance.', function() {
    const tests: [string, string, number, number, number][] = [
      ['', '', 1, 1, 1],
      ['Hello', 'Hello', 1, 1, 1],
      ['abc', 'def', 0, 0, 0],
    ...
    tests.forEach(function([a, b, lig1Distance, lig2Distance, lig3Distance]) {
      approximately(lig2(a, b), lig2Distance, 0.01, `LIG2: ${a}, ${b}`);
      approximately(lig3(a, b), lig3Distance, 0.01, `LIG3: ${a}, ${b}`);
    });
  });
});
