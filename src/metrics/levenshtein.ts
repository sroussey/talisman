 ...
 *
 * @param a - The first string to process.
 * @param b - The second string to process.
 * @return The Levenshtein distance between a & b.
 */
function levenshteinForStrings(a: string, b: string): number {
  if (a === b)
    return 0;
 ...
 * @return The Levenshtein distance between a & b.
 */
export default function levenshtein<T>(a: Sequence<T>, b: Sequence<T>): number {

  // If the sequences are string, we use the optimized version
  if (typeof a === 'string')
    return levenshteinForStrings(a, b as string);
 ...
 * @param a - The first string to process.
 * @param b - The second string to process.
 * @return The Levenshtein distance between a & b or Infinity.
 */
function limitedLevenshteinForStrings(max: number, a: string, b: string): number {
  if (a === b)
    return 0;
 ...
 * @return The Levenshtein distance between a & b or Infinity.
 */
export function limited<T>(max: number, a: Sequence<T>, b: Sequence<T>): number {

  // If the sequences are string, we use the optimized version
  if (typeof a === 'string')
    return limitedLevenshteinForStrings(max, a, b as string);
