 ...
 */
/**
 * What should happen once a rule has been applied.
 */
type LancasterAction = 'INTACT' | 'CONTINUE' | 'STOP' | 'PROTECT';

const INTACT: LancasterAction = 'INTACT',
      CONTINUE: LancasterAction = 'CONTINUE',
      STOP: LancasterAction = 'STOP',
      PROTECT: LancasterAction = 'PROTECT';

/**
 * A rule: the suffix to match, its replacement & what to do next.
 */
type LancasterRule = [string, string, LancasterAction];

const RULES: Record<string, LancasterRule[]> = {
 ...
 * Helpers.
 */
function isStemAcceptable(stem: string): boolean {
 ...
 */
export default function lancaster(word: string): string {
