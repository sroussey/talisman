...
const complexSuffixing = (word: string): string => {
...
/**
 * A rule: the pattern to match, how the matching word should be shortened
 * (a replacement string, a number of characters to drop, a function or
 * nothing at all) & the rule's number.
 */
type UeaLiteRule = [RegExp, string | number | ((word: string) => string) | null, string];

const RULES: UeaLiteRule[] = [
  [/^\d+$/, null, '90.3'],
  [/^\w+-\w+$/, null, '90.2'],
  [/-/, null, '90.1'],
  [/_/, null, '90'],
 ...
 * @return An object containing the stemmed word and the applied
 *                         rule.
 */
/**
 * A stem along with the number of the rule that produced it.
 */
export interface UeaLiteResult {
  /** The number of the applied rule. */
  readonly rule: string;
  /** The resulting stem. */
  readonly stem: string;
}

export function withRule(word: string): UeaLiteResult {
  let stem = word;
...
export default function ueaLite(word: string): string {
