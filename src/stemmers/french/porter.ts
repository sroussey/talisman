 ...
 * Helpers.
 */
function findR1R2(word: string): [string, string] {
  let r1 = '',
      r2 = '';
...
function findRV(word: string): string {
  let rv = '';
...
function replaceLetter(stem: string, index: number, replacement: string): string {
  return stem.substring(0, index) + replacement + stem.slice(index + 1);
}

function suffixStem(stem: string, oldSuffix: string | number, newSuffix = ''): string {
 ...
 */
export default function porter(word: string): string {
