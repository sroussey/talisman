 ...
 * Helpers.
 */
function computeM(string: string): number {
 ...
 */
/**
 * A rule: the maximum measure of the resulting stem for the rule to apply,
 * the suffix to drop & what should replace it.
 */
type CarryRule = [number, string, string?];

const STEP1: CarryRule[] = [
  [0, 'issaient'],
  [0, 'ellement', 'el'],
  [0, 'issement'],
  [0, 'alement', 'al'],
 ...
 */
function applyRules(rules: CarryRule[], stem: string): string {
 ...
 * @return The resulting stem.
 */
export default function carry(word: string): string {
