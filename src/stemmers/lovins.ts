...
for (let i = 0, l = ENDINGS_LIST.length; i < l; i += 2)
  ENDINGS.push([ENDINGS_LIST[i] as RegExp, ENDINGS_LIST[i + 1] as string]);

/**
 * Conditions.
 */
/**
 * A condition a stem must satisfy for the matching ending to be dropped.
 */
type LovinsCondition = (stem: string) => boolean;

const CONDITIONS: Record<string, LovinsCondition> = {
  A: () => true,
 ...
 */
export default function lovins(word: string): string {
