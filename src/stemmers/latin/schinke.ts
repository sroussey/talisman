...
const VERB_SUFFIXES: [RegExp, string?][] = [
  [/iuntur$/, '$1i'],
  [/erunt$/, '$1i'],
  [/untur$/, '$1i'],
  [/iunt$/, '$1i'],
 ...
 * Helpers.
 */
function normalizeLetters(stem: string): string {
  return stem
    .replace(/j/g, 'i')
    .replace(/v/g, 'u');
}
 ...
 * @return The resulting stems (noun & verb).
 */
/**
 * The two stems the Schinke algorithm computes for a single word.
 */
export interface SchinkeStems {
  /** The stem to use if the word is a noun. */
  readonly noun: string;
  /** The stem to use if the word is a verb. */
  readonly verb: string;
}

export default function schinke(word: string): SchinkeStems {
