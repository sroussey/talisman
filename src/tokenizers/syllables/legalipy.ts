 ...
 */
import {updateFrequencies, relative} from '../../helpers/frequencies.js';
import type {Frequencies} from '../../types.js';
 ...
 */
/**
 * The model a trained tokenizer exports, so that training does not have to be
 * repeated.
 */
export interface LegalipyModel {
  /** The onsets kept by the tokenizer. */
  readonly onsets: string[];
}

export class LegalipyTokenizer {
  /** Frequencies of the onsets seen during training, dropped once finalized. */
  private frequencies: Frequencies | null;

  /** The onsets used to cut words into syllables. */
  onsets: Set<string>;

  /** Whether the tokenizer's training has been finalized. */
   ...
   *
   * @return An object containing the necessary metadata.
   */
  export(): LegalipyModel {
    return {
 ...
 *
 * @param tokens - Word tokens.
 * @return A list of word tokenized as syllables.
 */
export default function defaultTokenizer(tokens: string[]): string[][] {
