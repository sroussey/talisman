...
import {findall} from '../../helpers/index.js';
...
const ORTHO_MAP: Record<string, number> = {
...
export class PunktLanguageVariables {
  ...
  readonly sentenceEndCharacters: Set<string>;

  /** Internal punctuation. */
  readonly internalPunctuation: Set<string>;

  /** Pattern used to realign boundaries. */
  readonly reBoundaryRealignment: RegExp;

  /** Characters excluded from starting a word token. */
  readonly reWordStart: RegExp;

  /** Characters that cannot appear within a word. */
  readonly reNonWordCharacters: RegExp;

  /** Multi-character punctuation, such as hyphens & ellipsis. */
  readonly reMultiCharacterPunctuation: RegExp;

  /** Pattern used to tokenize words. */
  readonly reWordTokenizer: RegExp;

  /** Pattern used to capture the context of a period. */
  readonly rePeriodContext: RegExp;
   ...
   */
  tokenizeWords(string: string): string[] {
 ...
 * @constructor
 */
export class PunktParameters {
  /** Word types known to be abbreviations. */
  abbreviationTypes: Set<string>;

  /** Known common collocations, as `first‡second` pairs. */
  collocations: Set<string>;

  /** Word types often found at the beginning of a sentence. */
  sentenceStarters: Set<string>;

  /** Orthographic contexts a word type appears in, as a bitmask. */
  orthographicContext: Record<string, number>;
   ...
   * @param flag - The context's flag.
   * @return Returns itself for chaining purposes.
   */
  addOrthographicContext(type: string, flag: number): this {
 ...
 */
/**
 * A span of text, as a `[start, end]` pair of indices.
 */
 ...
 *
 * @constructor
 * @param string - The token's string.
 * @param object - Custom flags.
 */
/**
 * The annotations a token can be created with, or decorated with during the
 * passes of the algorithm.
 */
export interface PunktTokenFlags {
  /** Whether the token starts a line. */
  lineStart?: boolean;
  /** Whether the token starts a paragraph. */
  paragraphStart?: boolean;
  /** Whether the token ends a sentence. */
  sentenceBreak?: boolean;
  /** Whether the token is an ellipsis. */
  ellipsis?: boolean;
  /** Whether the token is a known abbreviation. */
  abbreviation?: boolean;
}

export class PunktToken implements PunktTokenFlags {
  /** The token's string. */
  ...
  /** The token's type: its lowercase form, with numbers normalized. */
  readonly type: string;

  /** Whether the token is made of two or more periods. */
  readonly isEllipsis: boolean;

  /** Whether the token is a number. */
  readonly isNumber: boolean;

  /** Whether the token is a single-letter initial. */
  readonly isInitial: boolean;

  /** Whether the token is purely alphabetic. */
  readonly isAlpha: boolean;

  /** Whether the token holds at least one non-punctuation character. */
  readonly isNonPunctuation: boolean;

  /** Whether the token starts with an alphabetic character. */
  readonly isInitialAlpha: boolean;
  ...
  constructor(string: string, params: PunktTokenFlags = {}) {
 ...
 * @param [options.vars] - Language variables.
 * @param [options.params] - Parameters
 */
/**
 * Options shared by the Punkt trainer & tokenizer.
 */
export interface PunktBaseOptions {
  /** Language variables to use. */
  readonly vars?: PunktLanguageVariables;
  /** Parameters to use. */
  readonly params?: PunktParameters;
}

export class PunktBaseClass {
  /** The parameters used to detect sentence boundaries. */
  params: PunktParameters;

  /** The language variables used to tokenize the text. */
  readonly vars: PunktLanguageVariables;

  constructor(options?: PunktBaseOptions) {
    const {
   ...
   */
  tokenizeWords(text: string): PunktToken[] {
   ...
   *
   * @param tokens - The tokens to annotate.
   * @return Returns itself for chaining purposes.
   */
  _annotateFirstPass(tokens: PunktToken[]): this {
 ...
 * @param a - Count of <a>.
 * @param b - Count of <b>.
 * @param ab - Count of <ab>.
 * @param N - Number of elements in the distribution.
 * @return The log-likelihood.
 */
function dunningLogLikelihood(a: number, b: number, ab: number, N: number): number {
  const p1 = b / N,
        p2 = 0.99;
 ...
 * @return The log-likelihood.
 */
function colLogLikelihood(a: number, b: number, ab: number, N: number): number {
  const p = b / N,
        p1 = ab / a,
        p2 = (b - ab) / (N - a);
 ...
 * @constructor
 * @param [options] - Instantiation options.
 * @param [options.verbose] - Should the trainer log information?
 */
/**
 * Options of the Punkt trainer.
 */
export interface PunktTrainerOptions extends PunktBaseOptions {
  /** Whether the trainer should log what it finds. */
  readonly verbose?: boolean;
}

/**
 * A candidate found by the trainer, along with its log-likelihood.
 */
type Candidate = [string, number];

/**
 * An abbreviation candidate: its type, its score & whether it should be added
 * to the known abbreviations rather than removed from them.
 */
type AbbreviationCandidate = [string, number, boolean];

export class PunktTrainer extends PunktBaseClass {
  /** Whether the trainer logs what it finds. */
  private readonly verbose: boolean;

  /** Frequency of each case-normalized token type in the training data. */
  readonly typeFdist: FrequencyDistribution;

  /** Frequency of the bigrams whose first word ends in a period. */
  readonly collocationFdist: FrequencyDistribution;

  /** Frequency of the tokens following a sentence break. */
  readonly sentenceStarterFdist: FrequencyDistribution;

  /** Number of words ending in a period in the training data. */
  periodTokenCount: number;

  /** Number of sentence breaks identified during training. */
  sentenceBreakCount: number;

  /** Whether training was finalized. */
  finalized: boolean;

  /** Cutoff of the log-likelihood ratio for abbreviations. */
  ABBREV: number;

  /** Whether to ignore the length penalty applied to abbreviations. */
  IGNORE_ABBREV_PENALTY: boolean;

  /** Number of occurrences under which an abbreviation is deemed rare. */
  ABBREV_BACKOFF: number;

  /** Cutoff of the log-likelihood ratio for collocations. */
  COLLOCATION: number;

  /** Cutoff of the log-likelihood ratio for sentence starters. */
  SENT_STARTER: number;

  /** Whether to consider every bigram as a potential collocation. */
  INCLUDE_ALL_COLLOCS: boolean;

  /** Whether to consider abbreviations as potential collocations. */
  INCLUDE_ABBREV_COLLOCS: boolean;

  /** Number of occurrences under which a collocation is dropped. */
  MIN_COLLOC_FREQ: number;

  constructor(options?: PunktTrainerOptions) {
    const {
      verbose = false
    } = options || {};
   ...
   *
   * @param tokens - Training tokens.
   * @return Returns itself for chaining purposes.
   */
  _getOrthographyData(tokens: PunktToken[]): void {
   ...
   * @return Returns a triple containing the following:
   ...
   */
  _reclassifyAbbreviationType(type: string): AbbreviationCandidate | null {
    let isAdd: boolean;
   ...
   *
   * @param currentToken - The token.
   * @param nextToken - The next token.
   */
  _isRareAbbreviationType(currentToken: PunktToken, nextToken: PunktToken): boolean {
   ...
   *
   * @param firstToken - first The token.
   * @param secondToken - The second token.
   */
  _isPotentialCollocation(firstToken: PunktToken, secondToken: PunktToken): boolean {
    return !!(
    ...
    for (let i = 0, l = types.length; i < l; i++) {
      ...
      if (type1Count > 1 &&
        ...
        if (ll >= this.COLLOCATION &&
          ...
          results.push([hash, ll] as Candidate);
      }
    }
   ...
   */
  _isPotentialSentenceStarter(token: PunktToken, previousToken: PunktToken): boolean {
   ...
   * @return An array of results.
   */
  _findSentenceStarters(): Candidate[] {
    ...
    for (let i = 0, l = types.length; i < l; i++) {
      ...
      if (ll >= this.SENT_STARTER &&
        ...
        results.push([type, ll] as Candidate);
      }
    }
   ...
   * @param finalize - Whether to finalize the training or not.
   * @return Returns itself for chaining purposes.
   */
  train(text: string, finalize = true): this {
 ...
 *
 * @constructor
 * @param params - Parameters to use to perform tokenization.
 */
export class PunktSentenceTokenizer extends PunktBaseClass {
  /** Punctuation marks a sentence cannot start with. */
  private readonly PUNCTUATION: Set<string>;

  constructor(params: PunktParameters) {
    super();
   ...
   * @return The decision
   */
  _orthographicHeuristic(token: PunktToken): boolean | 'unknown' {
   ...
   *
   * @param tokens - Tokens to annotate.
   * @return Returns itself for chaining.
   */
  _annotateSecondPass(tokens: PunktToken[]): void {
   ...
   * @return Returns itself for chaining.
   */
  _annotateTokens(tokens: PunktToken[]): this {
   ...
   */
  _textContainsSentenceBreak(text: string): boolean {
    const tokens = this.tokenizeWords(text);
   ...
   * @return The slices.
   */
  _slicesFromText(text: string): Span[] {
    const slices: Span[] = [],
   ...
   *
   * @param text - Text to realign.
   * @param slices - Slices of text.
   * @return Realigned pieces.
   */
  _realignBoundaries(text: string, slices: Span[]): Span[] {
    const realigned: Span[] = [];
    ...
    for (let i = 0, l = slices.length; i < l; i++) {
      ...
      const realignedSlice: Span = [slice1[0] + realign, slice1[1]];
      ...
      if (match) {
        ...
        realign = (match.index as number) + match[0].length;
      }
      else {
        realign = 0;
        if (text.substring(...realignedSlice))
   ...
   * @param realignBoundaries - Should the tokenizer realign
   *                                        boundaries?
   * @return The array of sentences.
   */
  spanTokenize(text: string, realignBoundaries = true): Span[] {
   ...
   * @return The array of sentences.
   */
  tokenize(text: string, realignBoundaries = true): string[] {
