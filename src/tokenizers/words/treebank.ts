 ...
 * Rules.
 */
const STARTING_QUOTES: [RegExp, string][] = [
  [/^"/g, '``'],
  [/(``)/g, ' $1 '],
  [/([ (\[{<])"/g, '$1 `` ']
];
...
const PARENS_BRACKETS: [RegExp, string][] = [
  [/([\]\[\(\)\{\}\<\>])/g, ' $1 '],
  [/--/g, ' -- ']
];

const ENDING_QUOTES: [RegExp, string][] = [
  [/"/g, ' \'\' '],
  [/(\S)('')/g, '$1 $2 '],
  [/([^' ])('[sS]|'[mM]|'[dD]|') /g, '$1 $2 '],
...
function applyRules(rules: [RegExp, string][], text: string): string {
