...
/**
 * A code emitted by a rule: either a single code, a list of alternative codes
 * or nothing at all.
 */
type DaitchCode = number | (number | null)[] | null;

/**
 * A rule: a pattern along with the codes to emit when it is the first letter,
 * when a vowel follows & in every other case.
 */
type DaitchRule = [RegExp | null, DaitchCode, DaitchCode, DaitchCode];

const RULES: Record<string, DaitchRule[]> = {
  A: [
    [/^(AI|AJ|AY)/, 0, 1, null],
    [/^AU/, 0, 7, null],
    [null, 0, null, null]
...
function permutations(code: NonNullable<DaitchCode>[]): string[] {
  const codes = [''];
 ...
 * @return The Daitch-Mokotoff code.
 *
...
export default function daitchMokotoff(name: string): string[] {
  ...
  const code: NonNullable<DaitchCode>[] = [];
