 ...
 * Maps.
 */
const binary = (str: string | number): number => typeof str === 'string' ? parseInt(str, 2) : str,
      charCode = (char: string): number => char.charCodeAt(0),
      charArray = (str: string): number[] => str.split('').map(charCode);
...
export default function eudex(word: string): bigint {
  ...
  let firstByteValue = 0;

  if (entry < LETTERS)
    firstByteValue = INJECTIVE_PHONES[entry];
