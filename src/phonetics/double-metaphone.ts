...
function isSlavoGermanic(string: string): boolean {
...
/**
 * A code emitted by one of the lookups: either a single letter, a list of
 * letters to append, or nothing at all.
 */
type MetaphoneCode = string | string[] | null;

/**
 * What a lookup returns: the primary code, the secondary one & the offset by
 * which the cursor should be advanced.
 */
type MetaphoneResult = [MetaphoneCode, MetaphoneCode, number];

/**
 * A letter lookup.
 */
type Lookup = (string: string, pos: number, lastIndex: number, length: number) => MetaphoneResult;
...
const LOOKUPS: Record<string, Lookup> = {
  ...
  C(string, pos, lastIndex, length) {
    ...
    if (string.substr(pos, 2) === 'CH') {
      return LOOKUPS.CH(string, pos, lastIndex, length);
    }
    ...
    if (string.substr(pos, 2) === 'CC' &&
      ...
      return LOOKUPS.CC(string, pos, lastIndex, length);
    }
  ...
  G(string, pos, lastIndex, length) {
    const nextLetter = string.substr(pos + 1, 1),
          nextPair = string.substr(pos + 1, 2);

    if (nextLetter === 'H') {
      return LOOKUPS.GH(string, pos, lastIndex, length);
    }

    if (nextLetter === 'N') {
      return LOOKUPS.GN(string, pos, lastIndex, length);
    }
  ...
  S(string, pos, lastIndex, length) {
    ...
    if (string.substr(pos, 2) === 'SH') {
      return LOOKUPS.SH(string, pos, lastIndex, length);
    }
    ...
    if (string.substr(pos, 2) === 'SC') {
      return LOOKUPS.SC(string, pos, lastIndex, length);
    }
  ...
  T(string, pos, lastIndex, length) {
    ...
    if (string.substr(pos, 2) === 'TH' ||
      ...
      return LOOKUPS.TH(string, pos, lastIndex, length);
    }
 ...
 * @return The double metaphone codes.
 *
