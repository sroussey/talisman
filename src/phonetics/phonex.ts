...
const INITIALS: [Set<string>, string][] = ([
  ...
  ['JG', 'G'],
  ['ZS', 'S']
] as [string, string][]).map(([letters, replacement]) => [new Set(letters), replacement]);

const B_SET = new Set('BPFV'),
 ...
 * @return The Phonex code.
 *
 * @throws {Error} The function expects the name to be a string.
 */
export default function phonex(name: string): string {
