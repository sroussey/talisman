...
//       at all.
/**
 * An encoder taking the groups captured by a rule's pattern and returning the
 * emitted code along with what remains to be encoded.
 */
type SonnexEncoder = (...groups: string[]) => [string, string];

/**
 * A rule: a literal word or a pattern, along with its encoding.
 */
type SonnexRule = [string | RegExp, string | SonnexEncoder];

const RULES: Record<string, SonnexRule[]> = {
  a: [
    ['a', 'a'],
    ['aient', 'E'],
    ['ain', '1'],
 ...
 * @return The Sonnex code.
 *
 * @throws {Error} The function expects the word to be a string.
 */
export default function sonnex(word: string): string {
