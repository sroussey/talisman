 ...
 * Rules.
 */
const FIRST_PREPROCESSING: [RegExp, string][] = [
  [/O[O]+/g, 'OU'],
  [/SAOU/g, 'SOU'],
  [/OES/g, 'OS'],
  [/CCH/g, 'K'],
...
const SECOND_PREPROCESSING: [RegExp, string][] = [
  [/OIN[GT]$/g, 'OIN'],
  [/E[RS]$/g, 'E'],
  [/(C|CH)OEU/g, 'KE'],
  [/MOEU/g, 'ME'],
...
const FIRST_ENDINGS: [RegExp, string][] = [
  [/USIL$/, 'USI'],
  [/X$|[TD]S$|[DS]$/, ''],
  [/([^KL]+)T$/, '$1'],
...
const SECOND_ENDINGS: [RegExp, string][] = [
  [/TIL$/, 'TI'],
  [/LC$/, 'LK'],
  [/L[E]?[S]?$/, 'L'],
  [/(.+)N[E]?[S]?$/, '$1N'],
 ...
 * @return The phonetic code.
 *
 * @throws {Error} The function expects the word to be a string.
 */
export default function phonetic(word: string): string {
